...
def parse_host_discovery(result: NmapResult) -> list[tuple[str, str | None]]:
    """
    Parse the output of host_discovery() (nmap -sn) into a list of
    (ip, hostname) tuples. hostname is None when nmap couldn't
    resolve one for that host.
    """
    ...
    for line in result.stdout.splitlines():
        ...
        if remainder.endswith(")") and "(" in remainder:
            hostname, _, ip_part = remainder.rpartition("(")
